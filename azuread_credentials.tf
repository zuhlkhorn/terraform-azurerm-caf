...
module "azuread_credentials" {
  ...
  resources = {
    application = {
      ...
      client_id = can(each.value.azuread_application.client_id) ? each.value.azuread_application.client_id : local.combined_objects_azuread_applications[try(each.value.azuread_application.lz_key, local.client_config.landingzone_key)][each.value.azuread_application.key].application_id
    }
  }
}
