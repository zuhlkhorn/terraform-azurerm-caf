...
resource "azurerm_role_assignment" "for" {
  ...
  role_definition_name = each.value.mode == "built_in_role_mapping" ? each.value.role_definition_name : module.custom_roles[each.value.role_definition_name].role_definition_name
